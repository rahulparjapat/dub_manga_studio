...
from ..adapt import batch_manager as BM
from ..adapt import glossary, prompts, providers
...
from ..export.subtitle_mask import MASK_COLORS, MASK_TYPES
...
def build_tab1():
    ...
    with gr.Tab("1 · Ingest & Transcribe"):
        ...
        with gr.Row():
            warm_btn = gr.Button(
                "🔥 Download & load Whisper on GPU now " "(warm up while you upload)",
                variant="secondary",
            )
            warm_status = gr.Markdown(
                "_Whisper not warmed yet — first transcribe " "will load the model._"
            )
        with gr.Accordion("① Add source video (upload / folder / Drive)", open=True):
            upload = gr.File(
                label="Browser upload video (best for small/medium files)", file_types=["video"]
            )
            gr.Markdown(
            ...
            with gr.Row():
                folder_pick = gr.Dropdown(
                    ingest.list_input_videos(), label="…or pick from data/input/"
                )
        ...
        with gr.Accordion("② Transcribe", open=True):
            ...
            gr.Markdown(
                "_Control how much audio Whisper transcribes per cue. "
                "Bigger = fewer, longer cues (≈ 60 ÷ seconds cues per minute). "
                "e.g. 30s ≈ 2 cues/min, 15s ≈ 4 cues/min._"
            )
            chunk_len = gr.Slider(
                5, 45, value=_chunk_default, step=1, label="Whisper chunk length (seconds per cue)"
            )
        ...
        def do_ingest(name, up, folder, drive_url):
            ...
            path = None
            if up is not None:
                path = ingest.store_uploaded(
                    up.name if hasattr(up, "name") else up, pid, Path(getattr(up, "name", up)).name
                )
            elif folder:
        ...
        def do_warm(progress=gr.Progress()):
            ...
            if not r.get("ok"):
                return (
                    f"⚠ Whisper warm-up did not confirm GPU: {r.get('error', '')} "
                    f"(transcribe still works; check Live Logs)."
                )
            dev = r.get("device", "?")
            if dev == "cpu":
                return (
                    "⚠ Whisper loaded on **CPU** (GPU unavailable) — see Live Logs "
                    "for the reason. Transcription will be slow."
                )
            return (
                f"✅ Whisper is **warm on {dev}** ({r.get('compute','')}). "
        ...
        def do_transcribe(name, lang, with_ts, chunk_s, progress=gr.Progress()):
            ...
            SF.mark_stage(pid, "ingest")
            SF.mark_stage(pid, "transcribe")
        ...
        ingest_btn.click(
            do_ingest, [proj, upload, folder_pick, drive], [vpreview, status, transcript_box]
        )
        warm_btn.click(do_warm, outputs=warm_status)
        transcribe_btn.click(do_transcribe, [proj, src_lang, show_ts, chunk_len], transcript_box)
...
def build_tab2():
    with gr.Tab("2 · Script & Adaptation"):
        ...
        with gr.Row():
            ...
            target = gr.Dropdown(H.target_choices(), label="Target", value="hinglish_devanagari")
            dub_model = gr.Dropdown(
                H.model_choices(), value="indicf5", label="Dubbing model (forwarded to Tab 3)"
            )
            refresh = gr.Button("↻")
        with gr.Row():
        ...
        gr.Markdown(
            "_Tip: press the button above while you review/adapt the script — "
        ...
        with gr.Tabs():
            ...
            with gr.Tab("B · Import SRT"):
                srt_file = gr.File(label="Target SRT (match source cue count)", file_types=[".srt"])
                import_srt = gr.Button("Import SRT as adaptation")
            with gr.Tab("C/D · AI adaptation"):
                with gr.Row():
                    provider = gr.Dropdown(providers.PROVIDERS, value="gemini", label="Provider")
                    model = gr.Dropdown(
                ...
                with gr.Row():
                    style = gr.Dropdown(
                        H.style_choices(), value="Engaging YouTube Hinglish", label="Style"
                    )
                    retention = gr.Dropdown(
                        prompts.retention_choices(),
                        value="Full Duration Dub",
                        label="🎬 Adaptation mode (duration & pacing)",
                    )
                    engagement = gr.Dropdown(
                        prompts.engagement_choices(),
                ...
                with gr.Accordion("💾 Full-setup presets (one-click per new video)", open=False):
                    gr.Markdown(
                        "_Save your whole setup (target + style + retention + "
                    ...
                    with gr.Row():
                        ...
                        setup_pick = gr.Dropdown(prompts.list_setup_presets(), label="Load preset")
                ...
                with gr.Accordion("🎭 Emotions (model-aware, ON by default)", open=False):
                    add_emotions = gr.Checkbox(
                        True,
                        label="Add emotions for selected TTS model "
                ...
                eff_prompt = gr.Textbox(
                    label="Exact adaptation prompt + cue payload preview", lines=18
                )
        ...
        def do_refresh():
            return (
                gr.update(choices=H.list_projects()),
        ...
        def do_effective(p, t, s, pp, retn, eng, dubmid, emo_on, emo_free):
            ...
            sysp = prompts.build_effective_prompt(
                t, s, pp, glossary=g, retention=retn, engagement=eng
            )
            ...
            preview = (
                Q.build_cue_payload(cues[:5])
                if cues
                else "(Select a transcribed project to preview cue payload.)"
            )
            return (
        ...
        def do_refresh_models(prov, ft, ff, fj, q):
            ...
            return [
                [r["id"], r["provider"], r["text"], r["free"], r["json"], r["context"], r["notes"]]
                for r in rows
            ]
        ...
        def _translate_batch(
            ...
            dubmid,
            retn="",
            eng="Natural Commentary",
        ):
            from ..adapt import emotions as EMO
            from ..adapt import quality as Q

            tr = json.loads(
                (project_dir(pid) / "transcript" / "transcript.json").read_text(encoding="utf-8")
            )
            ...
            sysp = prompts.build_effective_prompt(
                t, st, pp, glossary=g, retention=retn, engagement=eng
            )
        ...
        def do_translate_all(p, t, prov, mdl, st, pp, ag, emo_on, emo_free, dubmid, retn, eng):
            ...
            for b in plan["batches"]:
                if b["status"] == "Done":
                    continue
                if not _translate_batch(
                    pid, t, b, prov, mdl, st, pp, ag, emo_on, emo_free, dubmid, retn, eng
                ):
                    break
        ...
        def do_list_versions(p, t, bn):
            ...
            return (
                gr.update(choices=[str(v) for v in vers], value=str(vers[-1])),
        ...
        def _count_note(pid, n_lines):
            ...
            return (
                f" ⚠️ WARNING: {n_lines} lines but {n_cues} source cues. "
        ...
        def do_import_srt(p, t, srt):
            ...
            return (
                "\n".join(lines),
                f"Imported {len(lines)} lines from SRT into Assembled box.{note}",
            )
        ...
        def do_backcheck(p, t, prov, mdl, text):
            ...
            if len(adapted) != len(cues):
                note = (
                    f"⚠️ Note: {len(adapted)} adapted lines vs {len(cues)} "
                    f"source cues — checking the overlapping range.\n"
                )
            else:
                note = ""
            payload = Q.build_backcheck_payload(cues, adapted)
            r = providers.adapt(prov, mdl, Q.BACKCHECK_INSTRUCTIONS, payload, want_json=True)
            if not r.get("ok"):
        ...
        def _duration_wps(t, mid):
            return (
                3.0 if mid == "voxcpm2" and t in ("hinglish_roman", "hinglish_devanagari") else 2.6
            )
        ...
        def do_duration_dashboard(p, t, mid, text):
            ...
            return (
                f"**Source cue duration:** {fit['source_seconds']/60:.1f} min  ·  "
                f"**Predicted narration:** {fit['predicted_seconds']/60:.1f} min  ·  "
                f"**Under-length cues:** {len(short)}/{len(cues)}  ·  "
                f"**Words:** {fit['words']}\n\n"
                + (
                    f"First short cues: {[r['idx']+1 for r in short[:20]]}"
                    if short
                    else "✅ All cues meet the selected duration target."
                )
                + duplicate_note
            )
        ...
        def do_repair_short(p, t, mid, text, prov, mdl, progress=gr.Progress()):
            ...
            prompt = (
                "You repair UNDER-LENGTH narration cues for a duration-faithful manga dub. "
            ...
            for row, replacement in zip(short, repaired, strict=True):
                if replacement.strip():
                    lines[row["idx"]] = replacement.strip()
            progress(1.0, desc="Duration repair complete")
            return "\n".join(lines), f"✅ Repaired {len(short)} under-length cues." + (
        ...
        def do_forward(p, t, mid, text):
            ...
            if n_cues is not None and len(lines) != n_cues:
                return (
                    f"⛔ Not forwarded: {len(lines)} narration lines but {n_cues} "
            ...
            if tr_path.exists():
                ...
                words = sum(len(re.findall(r"\b[\w'-]+\b", ln, flags=re.UNICODE)) for ln in spoken)
                ...
                target_wps = (
                    3.0
                    if (mid == "voxcpm2" and t in ("hinglish_roman", "hinglish_devanagari"))
                    else 2.6
                )
            ...
            ver = PKG.forward_package(
                safe_name(p),
                t,
                {"target": t, "narration_lines": lines, "dubbing_model": mid, "source": "tab2"},
            )
            from ..common import stageflow as SF
            ...
            SF.mark_stage(safe_name(p), "adaptation")
            SF.mark_stage(safe_name(p), "forward")
            SF.record_transcript_fingerprint(safe_name(p))  # L-3: lock the contract
            return (
                f"✅ Forwarded {len(lines)} lines to Dubbing as package {ver} "
        ...
        refresh_models.click(
            do_refresh_models, [provider, f_text, f_free, f_json, search], model_table
        )
        ...
        create_plan.click(do_create_plan, [proj, target, main_batches], [batch_table, status])
        ...
        dub_model.change(
            do_emo_note, [dub_model, add_emotions], [emo_note, add_emotions, ai_free_emo]
        )
        add_emotions.change(
            do_emo_note, [dub_model, add_emotions], [emo_note, add_emotions, ai_free_emo]
        )
        ...
        set_active.click(
            do_set_active, [proj, target, sel_batch, active_ver], [batch_table, status, batch_view]
        )
        load_assembled.click(do_load_assembled, [proj, target], [assembled, status])
        duration_dashboard_btn.click(
            do_duration_dashboard, [proj, target, dub_model, assembled], status
        )
        repair_short_btn.click(
            do_repair_short,
            [proj, target, dub_model, assembled, provider, model],
            [assembled, status],
        )
        ...
        def do_prewarm(mid, progress=gr.Progress()):
            ...
            from ..dubbing.installer import install_model, is_installed

            if not mid:
                yield "Pick a dubbing model first."
                return
            ...
            if r.get("ok"):
                ...
                yield (
                    f"❌ Install failed: {r.get('message','?')}\n\n"
        ...
        backcheck_btn.click(do_backcheck, [proj, target, provider, model, assembled], backcheck_out)
        ...
        def do_save_setup(nm, tgt, sty, retn, emo_on, emo_free, pp):
            msg = prompts.save_setup_preset(
                nm,
                {
                    "target": tgt,
                    "style": sty,
                    "retention": retn,
                    "add_emotions": bool(emo_on),
                    "ai_free_emo": bool(emo_free),
                    "project_prompt": pp,
                },
            )
        ...
        save_setup.click(
            do_save_setup,
            [setup_name, target, style, retention, add_emotions, ai_free_emo, project_prompt],
            [setup_status, setup_pick],
        )
        load_setup.click(
            do_load_setup,
            setup_pick,
            [target, style, retention, add_emotions, ai_free_emo, project_prompt, setup_status],
        )
...
def build_voice_design_tab():
    ...
    with gr.Tab("3A · Voice Design Studio"):
        ...
        with gr.Row():
            target = gr.Dropdown(
                H.target_choices(), value="hinglish_devanagari", label="Narrator language"
            )
        ...
        with gr.Accordion("🎙 Narrator Persona", open=True):
            ...
            seed = gr.Textbox(
                label="Seed script and exact transcript",
                lines=3,
        ...
        with gr.Accordion("🔀 Clone Mode (controls narrator identity in long dubs)", open=False):
            gr.Markdown(
                "**Hi-Fi Clone** = prompt_wav + prompt_text + reference_wav → highest similarity, style prefix ignored.\n"
            ...
            gr.Radio(
                [
                    ...
                    "Hi-Fi Clone (max fidelity)",
                    "Controllable Clone (style control)",
                ],
                value="Hybrid Smart Routing (recommended)",
                label="Clone mode for VoxCPM2",
            )
            gr.Markdown(
                "_Hi-Fi needs reference transcript for exact cloning. Controllable uses your style prefix. "
        ...
        with gr.Row():
            ...
            promote = gr.Button(
                "⭐ Set selected candidate as global default narrator", variant="primary"
            )
        ...
        with gr.Accordion("⭐ Candidate Scoring & Consistency Test", open=False):
            gr.Markdown(
                "Rate candidates 0–100, then run the 5-line consistency suite to verify narrator stability across diverse lines."
            )
            with gr.Row():
                cand_score = gr.Slider(0, 100, value=80, step=1, label="Score (0–100)")
                cand_notes = gr.Textbox(
                    label="Notes",
                    placeholder="e.g. natural pace, clear endings, slight buzz at end",
                )
        ...
        with gr.Accordion("🔎 Reference & narrator checks", open=False):
            ...
            gr.Markdown(
                "A global default narrator is saved as `data/voices/default_voice.wav`. Your saved personal voice remains available in Dubbing Studio as a secondary source."
            )

        def do_generate(t, n, desc, text, progress=gr.Progress()):
            line = f"({(desc or '').strip()}) {(text or '').strip()}".strip()
            r = VOX.generate_candidates(
                t, "voxcpm2", count=int(n), text=line, progress=lambda m: progress(0.5, desc=str(m))
            )
            if not r.get("ok"):
        ...
        def do_consistency(cand, t, progress=gr.Progress()):
            ...
            r = VOX.run_consistency_test(
                t, "voxcpm2", cand, progress=lambda m: progress(0.5, desc=str(m))
            )
            if not r.get("ok"):
        ...
        save_profile.click(
            do_save_profile, [profile_name, target, persona, seed], [profile_status, profile_pick]
        )
        ...
        consistency_btn.click(
            do_consistency, [candidate, target], [consistency_status, consistency_player]
        )
    return {}
...
def build_tab3():
    ...
    with gr.Tab("3 · Chatterbox Dubbing"):
        ...
        with gr.Row():
            ...
            refresh = gr.Button("↻")
        with gr.Row():
            model = gr.Dropdown(H.model_choices(), label="Dubbing model", value="indicf5")
            license_note = gr.Markdown()
        gr.Markdown(
            "_Disk-frugal mode: the model **installs on first Dub** (evicting "
        ...
        with gr.Row():
            fwd_version = gr.Dropdown(
                [],
                label="Adaptation version (per language)",
                info="Each language keeps its own versions. "
                "Pick one, or leave blank for the latest.",
            )
            fwd_refresh = gr.Button("↻ Versions")
            forward_btn = gr.Button("📥 Load selected/latest forwarded script", variant="primary")
        ...
        with gr.Accordion("🎬 Intro & Outro (adds an opener + closer to every video)", open=False):
            with gr.Row():
                io_enabled = gr.Checkbox(
                    IO.DEFAULT_ENABLED, label="Add intro & outro (on by default)"
                )
                io_preset = gr.Dropdown(IO.preset_names(), value=IO.DEFAULT_PRESET, label="Preset")
        ...
        with gr.Accordion("Voice controls", open=True):
            with gr.Row():
                voice_mode = gr.Radio(
                    ["Auto default voice (consistent)", "Saved reference voice", "Built-in voice"],
                    value="Auto default voice (consistent)",
                    label="Voice source",
                )
            ...
            with gr.Row():
                ...
                voice_denoise_str = gr.Slider(
                    0.5, 2.0, value=1.0, step=0.1, label="Cleaning strength"
                )
            gr.Markdown(
                "_Save once — your voice is stored and reused for every future "
            ...
            with gr.Accordion(
                "🔊 Voice test lab — try default voices, save the best, " "delete the rest",
                open=False,
            ):
                with gr.Row():
                    vt_count = gr.Slider(
                        1, 6, value=3, step=1, label="How many samples to generate"
                    )
                    vt_text = gr.Textbox(
                ...
                gr.Markdown(
                    "**Saved voice library** (reuse for dubbing / delete before you start):"
                )
                with gr.Row():
            ...
            def _speed_note(v):
                if v < 0.85 or v > 1.25:
                    return (
                        "_⚠ Outside 0.85×–1.25× the voice can start to sound "
            ...
            with gr.Accordion(
                "⚡ VoxCPM2 FlashAttention 2 + batching " "(advanced, sm_80+ GPUs only)", open=False
            ):
                if _fa2_ok:
                    gr.Markdown(
                ...
                with gr.Row():
                    fa2_on = gr.Checkbox(
                        _fa2_ok, interactive=_fa2_ok, label="Use FlashAttention 2 for VoxCPM2"
                    )
                    vox_batch = gr.Slider(
                        1,
                        8,
                        value=4 if _fa2_ok else 1,
                        step=1,
                        interactive=_fa2_ok,
                        label="VoxCPM2 batch size (concurrent cues; sm_80+ only)",
                    )
                energy = gr.Dropdown(
                    ["natural", "expressive", "calm"], value="expressive", label="Narration energy"
                )
            ...
            with gr.Accordion("🎛 Advanced TTS controls (fine-tune voice; optional)", open=False):
                gr.Markdown(
                    "_The **Narration energy** preset above already sets good "
                ...
                with gr.Row():
                    adv_temperature = gr.Slider(
                        0.0, 1.5, value=0.0, step=0.05, label="Temperature (variation; 0 = preset)"
                    )
                    adv_steps = gr.Slider(
                        0,
                        30,
            ...
            with gr.Accordion("🔀 VoxCPM2 Clone Mode (identity vs emotion control)", open=False):
                gr.Markdown(
                    "**Hi-Fi Clone** = prompt_wav + prompt_text + reference_wav → highest voice similarity, style prefix ignored.\n"
                    "**Controllable Clone** = reference_wav + style prefix → emotion/style controllable.\n"
        ...
        with gr.Accordion("Live Render Pipeline (optional)", open=False):
            ...
            with gr.Row():
                ...
                gr.Slider(1, 6, value=2, step=1, label="Min free VRAM (GB)")
            with gr.Row():
        ...
        with gr.Accordion("⚡ One-Click Auto (Dub → Render → Final MP4)", open=True):
            gr.Markdown(
                "_Press once. Runs TTS + parallel video render + merge and gives "
            ...
            with gr.Accordion("🎭 Mask settings (when 'Hide Chinese subtitles' is on)", open=False):
                with gr.Row():
                    auto_mask_type = gr.Dropdown(
                        MASK_TYPES, value="Blur + dark band", label="Blur / mask type"
                    )
                    auto_mask_color = gr.Dropdown(
                        MASK_COLORS, value="black", label="Band / cover color"
                    )
                with gr.Row():
                ...
                with gr.Row():
                    auto_mask_strength = gr.Slider(
                        1, 40, value=10, step=1, label="Blur/pixelate strength"
                    )
                    auto_mask_opacity = gr.Slider(0, 1, value=0.6, step=0.05, label="Band opacity")
                    auto_mask_ptime = gr.Slider(0, 600, value=5, step=1, label="Preview time (s)")
            ...
            with gr.Accordion("🎵 BGM (when 'Add BGM' is on)", open=False):
                with gr.Row():
                    auto_bgm_file = gr.Dropdown(
                        [p.name for p in BGM.glob("*")], label="BGM file (from data/bgm/)"
                    )
                    auto_bgm_refresh = gr.Button("↻")
                with gr.Row():
                    auto_bgm_upload = gr.Audio(
                        label="…or upload a BGM track", type="filepath", sources=["upload"]
                    )
        ...
        def _fwd_version_choices(p, t):
            ...
            choices = [
                f"{d['version']} · {d['created']} · {d['lines']} lines · {d['model']}"
        ...
        def do_forward(p, t, ver_label):
            ...
            note = (
                f"✅ Loaded **{pkg.get('_version','?')}** for **{t}** "
        ...
        def do_dub(
            ...
            if not p:
                yield "Pick a project."
                return
            pid = safe_name(p)
            lines = [ln for ln in text.splitlines() if ln.strip()]
            if not lines:
                yield "No narration lines."
                return
            ...
            if mcfg.get("needs_ref_transcript") and not reference:
                yield (
                    f"❌ {mcfg['label']} needs a reference voice. Use 'Auto default "
                    f"voice (consistent)' (easiest), or 'Saved reference voice' with "
                    f"a saved/uploaded voice + its transcript."
                )
                return
            if mcfg.get("needs_ref_transcript") and reference and not (reftext or "").strip():
                yield (
                    f"⚠ {mcfg['label']} works best with a reference transcript. "
            ...
            strip_note = (
                f" (removed emotion tags from {n_stripped} line(s) — "
                f"{mcfg['label']} can't read them)"
                if n_stripped
                else ""
            )
            ...
            try:
                ...
                yield (
                    ...
                    f"Run:  pip install soundfile numpy  (then restart the app)."
                )
                return
            ...
            if float(adv_cfg_v) > 0:
                preset["cfg_value"] = float(adv_cfg_v)
                preset["cfg_weight"] = float(adv_cfg_v)
            if float(adv_temp) > 0:
            ...
            for i, ln in enumerate(lines):
                ...
                reqs.append(
                    GenRequest(
                        ...
                        reference_text=reftext or None,
                        preset=preset,
                        emotion_tags=tags or None,
                    ).to_json()
                )
            warn_prefix = ""
            if chk.warning:
            ...
            if live:
                ...
                if trj.exists() and srcs:
                    ...
                    for i in range(len(lines)):
                        ...
                        cue_objs.append(
                            TL.Cue(idx=i, src_start=seg["start"], src_end=seg["end"], text=lines[i])
                        )
                    ...
                    def render_group(g, group_cues):
                        ...
                        lst = EX.build_segments_concat(
                            src_video, sub_tl, work / f"g{g}", fast_copy=False
                        )
                    ...
                    _dub_jobs[job_id].update(
                        {
                            ...
                            "current_cue": i + 1,
                            "message": (
                                "Cue completed" if r.get("ok") else r.get("error", "Cue failed")
                            ),
                        }
                    )
            ...
            if job_id in _dub_jobs:
                _dub_jobs[job_id].update(
                    {
                        "state": "cancelled" if was_cancelled else "finishing",
            ...
            if pipe:
                if was_cancelled:
                    ...
                    while _t.time() < deadline:
                        ...
                        progress(
                            min(0.99, 0.96 + 0.03 * done_g / tot_g),
                            desc=f"Rendering video groups {done_g}/{tot_g}…",
                        )
                        yield (
                            f"🎬 Live render: {done_g}/{tot_g} groups done "
                            f"({state})… TTS {ok_count['n']}/{len(lines)} cues."
                        )
                        if state in ("done", "cancelled", "error") or (tot_g and done_g >= tot_g):
                            break
                        _t.sleep(2)
            ...
            if job_id in _dub_jobs:
                _dub_jobs[job_id].update(
                    {
                        "state": "cancelled" if was_cancelled else "completed",
                        "done": ok,
                        "message": "Dub cancelled" if was_cancelled else "Dub completed",
                    }
                )
            if ok == 0 and results:
            ...
            if pipe:
                snap = pipe.snapshot()
                live_note = (
                    f" Live render: {snap['completed_groups']}/"
            ...
            if was_cancelled:
                yield (
                    f"🛑 Dub cancelled — kept {ok}/{len(lines)} cues already done. "
                    f"Press Dub again to resume (finished cues are skipped). "
                    f"Disk free: {disk_free_gb():.1f} GB."
                )
            else:
                yield (
                    f"{warn_prefix}✅ Dubbed {ok}/{len(lines)} cues with "
        ...
        def do_save_voice(fp, nm, dn, dn_str, src_lang_v):
            ...
            if r.get("ok"):
                return (
                    r["message"],
                    gr.update(choices=choices, value=r["name"]),
        ...
        save_voice_btn.click(
            do_save_voice,
            [upload_voice, upload_voice_name, voice_denoise, voice_denoise_str, gr.State("Auto")],
        ...
        def do_vt_generate(p, t, mid, n, text, progress=gr.Progress()):
            ...
            r = VOX.generate_candidates(
                t, mid, count=int(n), text=text, progress=lambda m: progress(0.5, desc=str(m))
            )
            if not r.get("ok"):
        ...
        def do_vt_use(name):
            ...
            note = f"✅ Using '{name}' for the dub." + (
                ...
                if tr
                else " (audio-only clone — no transcript needed.)"
            )
            return (
                gr.update(value="Saved reference voice"),
                gr.update(value=name),
                gr.update(value=tr) if tr else gr.update(),
                note,
            )
        ...
        def do_vt_delete(name):
            ...
            return (
                gr.update(choices=saved, value=newsel),  # test-lab list
        ...
        vt_gen.click(
            do_vt_generate,
            [proj, target, model, vt_count, vt_text],
            [vt_samples, vt_player, vt_status],
        )
        vt_samples.change(do_vt_play, vt_samples, vt_player)
        vt_save.click(
            do_vt_save, [vt_samples, vt_save_name, vt_text], [vt_saved, ref_voice, vt_manage_status]
        )
        vt_saved_refresh.click(lambda: (_saved_choices()), outputs=vt_saved)
        vt_saved.change(do_vt_saved_play, vt_saved, vt_saved_player)
        vt_use.click(do_vt_use, vt_saved, [voice_mode, ref_voice, ref_transcript, vt_manage_status])
        vt_delete.click(
            do_vt_delete, vt_saved, [vt_saved, ref_voice, vt_saved_player, vt_manage_status]
        )
        ...
        def do_testing_toggle(on):
            ...
            if on:
                return (
                    "🧪 **Testing mode ON** — every model can load on this GPU "
        ...
        def do_save_override(p, t, text):
            ...
            ver = PKG.save_override(
                safe_name(p),
                t,
                base,
                {"target": t, "narration_lines": lines, "source": "dub_override"},
            )
        ...
        def do_auto(
            ...
            if not p:
                yield None, None, None, None, None, "Pick a project."
                return
            pid = safe_name(p)
            lines = [ln for ln in text.splitlines() if ln.strip()]
            if not lines:
                yield None, None, None, None, None, "No narration lines (load a forwarded script)."
                return
            ...
            if vmode == "Saved reference voice" and refv:
                ...
                dv = VOX.ensure_default_voice(t, mid)
                if not dv.get("ok"):
                    yield None, None, None, None, None, f"❌ {dv.get('message')}"
                    return
                reference = dv["path"]
                if not (reftext or "").strip():
                    reftext = dv["text"]
            if mcfg.get("needs_ref_transcript") and not reference:
                yield None, None, None, None, None, (
                    ...
                    f"(consistent)' or 'Saved reference voice'."
                )
                return
            # Strip emotion tags if the chosen model can't read them (never spoken aloud).
            from ..adapt import emotions as EMO
            ...
            mask_opts = {
                "type": m_type or "Blur + dark band",
                "color": m_color or "black",
                "x": int(m_x),
                "y": int(m_y),
                "w": int(m_w),
                "h": int(m_h),
                "strength": int(m_strength),
                "opacity": float(m_opacity),
            }
            ...
            _dub_jobs[job_id] = {
                ...
                "total": len(lines),
                "current_cue": None,
                "message": "Starting Auto pipeline…",
            }
            ...
            def _work():
                ...
                try:
                    result["r"] = run_auto(
                        ...
                        captions=a_caps,
                        mask=a_mask,
                        mask_opts=mask_opts if a_mask else None,
                        bgm_path=bgm_path,
                        instances=int(inst),
                        force_regenerate=bool(fregen),
                ...
                _dub_jobs[job_id].update(
                    {
                        "state": (
                            "completed"
                            if r.get("ok")
                            else ("cancelled" if r.get("cancelled") else "failed")
                        ),
                        "message": r.get("message", "Auto pipeline finished"),
                    }
                )
            if not r.get("ok"):
                yield None, None, None, None, None, f"❌ {r.get('message')}"
                return
            progress(1.0, desc="Done")
            yield (
                r["final"],
        ...
        def do_auto_bgm_save(upath, name):
            ...
            _sh.copy(upath, dst)
            return (
                gr.update(choices=[p.name for p in BGM.glob("*")], value=dst.name),
        ...
        def do_auto_mask_preview(p, t, mtype, x, y, w, h, strength_v, opacity_v, ptime, color_v):
            ...
            fc = build_mask_filter(
                mtype, x, y, w, h, int(strength_v), float(opacity_v), color=color_v
            )
            try:
                subprocess.run(
                    [
        ...
        auto_bgm_save.click(
            do_auto_bgm_save, [auto_bgm_upload, auto_bgm_name], [auto_bgm_file, auto_bgm_status]
        )
        ...
        forward_btn.click(
            do_forward, [proj, target, fwd_version], [script_box, model, fwd_version, progress]
        )
        ...
        def _io_delete(name):
            ...
            return (
                gr.update(choices=names, value=newsel),
                p.get("intro", ""),
                p.get("outro", ""),
                msg,
            )
        ...
        io_save.click(
            _io_save, [io_new_name, io_intro, io_outro], [io_preset, io_new_name, io_status]
        )
        io_delete.click(_io_delete, io_preset, [io_preset, io_intro, io_outro, io_status])
        ...
        dash_timer.tick(do_dash_tick, [proj, target, dash_auto], dashboard, concurrency_limit=None)
        ...
        dub_dashboard_timer.tick(
            do_persistent_dub_dashboard, [proj, target], dub_dashboard, concurrency_limit=None
        )
        dub_btn.click(
            do_dub,
            [
        ...
        def do_cancel_dub(p, t):
            ...
            if found:
                return (
                    "🛑 Cancel requested — the dub will stop after the current "
        ...
        cancel_dub_btn.click(do_cancel_dub, [proj, target], progress, concurrency_limit=None)
    return {}
...
def build_tab4():
    with gr.Tab("4 · Direct Text to Audio"):
        ...
        with gr.Row():
            target = gr.Dropdown(
                H.target_choices(), value="hinglish_devanagari", label="Language / script"
            )
            model = gr.Dropdown(H.model_choices(), value="indicf5", label="Model")
            style = gr.Dropdown(
                ["natural", "expressive", "calm"], value="expressive", label="Energy"
            )
        with gr.Row():
            voice_mode = gr.Radio(["Built-in", "Saved reference"], value="Built-in", label="Voice")
            ref_voice = gr.Dropdown([p.name for p in VOICES.glob("*.wav")], label="Reference voice")
        ...
        with gr.Accordion("AI adapt this text first (optional, uses credits)", open=False):
            ...
            with gr.Row():
                adapt_provider = gr.Dropdown(
                    providers.PROVIDERS, value="gemini", label="Adapt provider"
                )
        ...
        def do_go(n, txt, t, mid, st, vmode, refv, reftext, tg, use_ai, ap, am, spd):
            ...
            return (
                r["wav"],
                r.get("mp3"),
                r.get("final_text", "") if use_ai else "",
...
def build_tab5():
    ...
    with gr.Tab("5 · Subtitles & Export"):
        ...
        with gr.Row():
            ...
            target = gr.Dropdown(
                H.target_choices(), value="hinglish_devanagari", label="Target edition"
            )
            gr.Dropdown(ex.get("presets", []), value="YouTube Standard", label="Export preset")
        gr.Markdown("Current Export Summary will appear here.")
        with gr.Accordion("Timing & Video Length", open=True):
            timing = gr.Dropdown(
                ex.get("timing_modes", []), value=ex.get("default_timing"), label="Timing mode"
            )
            with gr.Row():
        ...
        with gr.Accordion("Add My Target Subtitles", open=False):
            captions_on = gr.Checkbox(
                False, label="Add target captions (burn my English/Hindi text)"
            )
            ...
            caption_src = gr.Radio(
                ...
                value="Derive from narration (no credits)",
                label="Caption source",
            )
            gr.File(label="Custom SRT")
            caption_preview_btn = gr.Button("Preview captions (first 10 lines)")
            caption_preview = gr.Textbox(
                label="Caption preview (retimed to final timeline)", lines=8
            )
        with gr.Accordion("Hide Original Chinese Burned-In Subtitles", open=False):
            ...
            with gr.Row():
                ...
                mask_color = gr.Dropdown(MASK_COLORS, value="black", label="Band / cover color")
            with gr.Row():
            ...
            gr.Markdown(
                "_Move the sliders and click Preview to see the yellow box "
        ...
        with gr.Accordion("Audio & BGM", open=False):
            ...
            with gr.Row():
                ...
                gr.Checkbox(True, label="True peak limiter (-1.5 dBTP)")
            with gr.Row():
                bgm_gain = gr.Slider(
                    -30, 0, value=-12, step=1, label="BGM level (dB) — lower = quieter music bed"
                )
                duck_depth = gr.Slider(
                    1,
                    20,
                    value=8,
                    step=1,
        ...
        with gr.Accordion("YouTube Title, Description & Tags", open=False):
            gr.Checkbox(False, label="Generate metadata")
            meta_lang = gr.Dropdown(
                ex.get("metadata_languages", []),
                value="Hinglish Devanagari Preferred",
            ...
            with gr.Row():
                dl_txt = gr.Button("TXT")
                dl_json = gr.Button("JSON")
                dl_csv = gr.Button("CSV")
        ...
        def do_export(
            ...
            if not mask:
                ...
                if man.exists():
                    ...
                    if cache_matches_timeline(groups, tline, timing_mode):
                        ...
                        used_cache = True
            if not used_cache:
                lst = exporter.build_segments_concat(str(vids[0]), tline, work, fast_copy=False)
            ...
            if mask:
                ...
                fc = build_mask_filter(
                    mtype, x, y, w, h, int(strength_v), float(opacity_v), color=mask_color_v
                )
            ...
            if bgm_on_v and bgm_file_v:
                ...
                fc = bgm_mix_filter(
                    duck=bool(duck_v), bgm_gain_db=float(bgm_gain_v), duck_ratio=float(duck_depth_v)
                )
                exporter.mux_audio_with_bgm(video_stage, audio_master, bgm_path, final, fc)
            elif loudnorm_v:
                from ..export.bgm import clean_dub_audio_filter

                exporter.mux_audio(
                    video_stage, audio_master, final, audio_filter=clean_dub_audio_filter()
                )
            else:
            ...
            return (
                ...
                str(script_path),
                str(q),
                _export_done_msg(pid, final.name),
            )

        def do_mask_preview(
            p, t, mtype, x, y, w, h, strength_v, opacity_v, ptime, color_v, box_only
        ):
            if not p:
                return None
            import subprocess
        ...
        def do_gen_meta(p, t, lang, prov):
            ...
            r = providers.adapt(
                prov, "", "You are a YouTube metadata expert.", prompt, want_json=True
            )
            if not r.get("ok"):
        ...
        def do_meta_dl(p, t, md, fmt):
            ...
            outdir.mkdir(parents=True, exist_ok=True)
            if fmt == "txt":
                f = outdir / "metadata.txt"
                f.write_text(META.to_txt(md), encoding="utf-8")
            elif fmt == "csv":
                f = outdir / "metadata.csv"
                f.write_text(META.to_csv(md), encoding="utf-8")
            else:
                f = outdir / "metadata.json"
                f.write_text(META.to_json(md), encoding="utf-8")
            return str(f)
        ...
        caption_preview_btn.click(do_caption_preview, [proj, target, timing], caption_preview)
        gen_meta.click(
            do_gen_meta,
            [proj, target, meta_lang, meta_provider],
            [meta_out, meta_file, meta_limits],
        )
        dl_txt.click(
            lambda p, t, md: do_meta_dl(p, t, md, "txt"), [proj, target, meta_out], meta_file
        )
        dl_json.click(
            lambda p, t, md: do_meta_dl(p, t, md, "json"), [proj, target, meta_out], meta_file
        )
        dl_csv.click(
            lambda p, t, md: do_meta_dl(p, t, md, "csv"), [proj, target, meta_out], meta_file
        )
        quick_export.click(
            do_export,
            [
...
def build_tab6():
    with gr.Tab("6 · Settings, Prompts, Keys & Cleanup"):
        ...
        with gr.Accordion("Storage & Cleanup (10 GB budget)", open=True):
            ...
            with gr.Row():
                unload_all_models_btn = gr.Button(
                    "⏏ Unload ALL models from VRAM", variant="secondary"
                )
            ...
            with gr.Row():
                session_toggle = gr.Checkbox(
                    True,
                    label="Session Mode (ON by default — models like VoxCPM2 "
            ...
            with gr.Row():
                ...
                cleanup_exit_btn = gr.Button(
                    "🧹 Cleanup for exit (drop under 10 GB)", variant="primary"
                )
            session_status = gr.Markdown()
            gr.Markdown("---")
            gr.Markdown(
                "**Evict a TTS model** (delete its venv + weights to free space). "
            ...
            targets = gr.CheckboxGroup(
                ...
                label="Other cleanup targets",
            )
            gr.Markdown(
                "_⚠ 'Projects' = your transcripts, dubbed cues & exports. "
        ...
        def do_disk_status():
            from ..common.diskmanager import (
                MODEL_PEAK_GB,
                WHISPER_RESIDENT_GB,
                disk_free_gb,
                installed_model_venvs,
            )

            inst = installed_model_venvs()
            eff = 10 - WHISPER_RESIDENT_GB
            lines = [
                f"**Disk free:** {disk_free_gb():.1f} GB / 10 GB budget",
        ...
        def do_unload_all_models():
            ...
            free = free_vram_gb()
            suffix = (
                f" GPU free: {free:.1f} GB." if free is not None else " GPU status unavailable."
            )
            if errors:
        ...
        def do_evict(mid):
            from ..common.diskmanager import disk_free_gb, evict_model

            freed = evict_model(mid)
            return f"Evicted {mid}: freed {freed:.1f} GB. Disk free: {disk_free_gb():.1f} GB."

        def do_evict_all():
            from ..common.diskmanager import disk_free_gb, evict_all_except
        ...
        def do_session_toggle(on):
            ...
            if on:
                return (
                    "✅ Session Mode ON — bigger models allowed temporarily. "
        ...
        def do_footprint():
            ...
            lines = [
                f"**Persistent footprint (survives Studio stop):** "
        ...
        def do_cleanup(sel, confirm_projects):
            ...
            if "Projects" in sel and not confirm_projects:
                return (
                    "⚠ 'Projects' selected but NOT confirmed. Your transcripts / dubbed "
            ...
            mp = {
                "Projects": P.PROJECTS,
                "Input files": P.INPUT,
                "Output files": P.OUTPUT,
                "Reference voices": P.VOICES,
                "BGM": P.BGM,
                "Model cache": P.HF_CACHE,
            }
            n = 0
            for s in sel:
                d = mp.get(s)
        ...
        save_style.click(
            lambda n, t: prompts.save_custom_style(n, t), [style_name, style_text], prompt_status
        )
        ...
        unload_all_models_btn.click(
            do_unload_all_models, outputs=vram_unload_status, concurrency_limit=None
        )
...
def build_logs_tab():
    ...
    with gr.Tab("7 · Live Logs"):
        ...
        log_box = gr.Textbox(
            label="studio.log (newest at bottom)",
            lines=26,
            value=LOG.tail_log(400),
            autoscroll=True,
            max_lines=26,
        )
