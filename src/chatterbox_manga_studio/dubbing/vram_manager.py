...
from ..common.config import active_profile, load_config, model_cfg
...
def check_model_fits(model_id: str, live_render: bool = False) -> VramCheck:
    ...
    if testing_mode():
        ...
        if need > budget:
            warn = (
                f"🧪 Testing mode: '{m.get('label', model_id)}' (~{need:.0f}GB) may "
    ...
    if total_need > budget:
        if live_render and (need <= budget):
            return VramCheck(
                ok=True,
                warning=(
                    f"'{m.get('label', model_id)}' (~{need:.0f}GB) + live video render "
        ...
        return VramCheck(
            ok=False,
            warning=(
                f"'{m.get('label', model_id)}' needs ~{need:.0f}GB but only "
