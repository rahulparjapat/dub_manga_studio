...
_STATE: dict[str, Any] = {"thread": None, "stop": None, "until": 0.0, "port": None}
...
def start(port: int | None = None, minutes: int = 90, interval_s: int = 60) -> str:
    ...
    def _loop():
        log.info(
            "Keep-alive ON for %d min (pinging %s every %ds). NOTE: does not "
    ...
    _STATE.update(
        thread=threading.Thread(target=_loop, daemon=True), stop=stop_ev, until=until, port=port
    )
    _STATE["thread"].start()
    return (
        f"✅ Keep-alive **started** for {minutes} min (pings the app every "
