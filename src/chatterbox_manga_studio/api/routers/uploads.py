...
async def validate_upload(request: UploadInitRequest):
    ...
    return OkResponse(
        data={"valid": suffix in VIDEO_EXTS, "supported_extensions": sorted(VIDEO_EXTS)}
    )
...
async def upload_chunk(upload_id: str, chunk: UploadFile = File(...), state=Depends(get_state)):
    ...
    return UploadChunkResponse(
        upload_id=upload_id, received_bytes=meta["received_bytes"], complete=False
    )
...
async def complete_upload(upload_id: str, state=Depends(get_state)):
    ...
    await state.storage.put_object(
        object_key, final.read_bytes(), content_type=meta.get("content_type")
    )
