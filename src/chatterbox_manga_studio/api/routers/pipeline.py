...
from ..dependencies import get_workflow as workflow_dependency
...
async def start_workflow(request: WorkflowStartRequest, state=Depends(get_state)):
    ...
    run = await state.workflow.create_run(
        state.pipeline_factory.definition(), wf_input, metadata=request.metadata
    )
    if request.idempotency_key:
        await state.storage.set_kv(
            f"idempotency:workflow:{request.idempotency_key}", run.id, ttl=24 * 3600
        )
...
async def restart_workflow(run_id: str, state=Depends(get_state)):
    ...
    new_run = await state.workflow.create_run(
        run.definition, run.input, metadata={**run.metadata, "restarted_from": run_id}
    )
...
async def reset_nodes(
    run_id: str, request: ResetNodesRequest, workflow=Depends(workflow_dependency)
):
    return _workflow_response(
        await reset_pipeline_nodes(
            workflow, run_id, request.node_ids, include_dependents=request.include_dependents
        )
    )
...
async def workflow_progress(run_id: str, workflow=Depends(workflow_dependency)):
    ...
    return OkResponse(
        data={
            ...
            "nodes": {k: v.model_dump(mode="json") for k, v in run.node_states.items()},
        }
    )
