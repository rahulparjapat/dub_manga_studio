...
class AuthBackend:
    def __init__(self, *, api_keys: str | None = None, jwt_secret: str | None = None, auth_required: bool | None = None) -> None:
        self.api_keys = parse_api_keys(api_keys if api_keys is not None else os.getenv("CMS_API_KEYS"))
        self.jwt_secret = jwt_secret if jwt_secret is not None else os.getenv("CMS_JWT_SECRET")
        self.auth_required = (os.getenv("CMS_AUTH_REQUIRED", "false").lower() == "true") if auth_required is None else auth_required
    ...
    async def authenticate(self, api_key: str | None, authorization: str | None) -> Principal:
        if api_key:
            ...
            if entry:
                role, scopes = entry
                return Principal(subject=f"api_key:{api_key[:8]}", scopes=scopes, auth_type="api_key", role=role)
        ...
        if authorization and authorization.startswith("Bearer "):
            ...
            if not self.jwt_secret:
                ...
                try:
                    ...
                    scopes = tuple(payload.get("scopes") or ROLE_POLICIES.get(role, ROLE_POLICIES["viewer"]).scopes)
                    return Principal(subject=str(payload.get("sub", "jwt")), scopes=scopes, auth_type="jwt", role=role)
                except Exception as exc:
                    if self.auth_required:
                        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=f"invalid JWT: {exc}") from exc
        if self.auth_required:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="authentication required")
        return Principal(subject="anonymous", scopes=("read",), auth_type="anonymous", role="viewer")
