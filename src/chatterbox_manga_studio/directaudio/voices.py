...
from ..common.config import default_model_for_target, preset_for_style
...
DEFAULT_VOICE_PERSONA = (
    "A warm early-twenties Indian male narrator, medium-low clear voice, "
...
def _save_profiles(profiles: dict):
    ...
    NARRATOR_PROFILES_FILE.write_text(
        json.dumps(profiles, indent=2, ensure_ascii=False), encoding="utf-8"
    )


def save_narrator_profile(
    name: str, target: str, persona: str, seed_text: str, model: str = "voxcpm2"
) -> dict:
    ...
    try:
        ...
        if auto_transcribe:
            try:
                ...
                tr = transcribe_clip(
                    str(dst), source_language=source_language, tts_loaded_model=tts_loaded_model
                )
        ...
        log.info(
            "saved reference voice: %s%s%s",
            dst.name,
            " (trimmed)" if trim_note else "",
        ...
        return {
            ...
            "freed_tts": reloaded,
            "message": (
                f"Saved reference voice '{dst.name}'." + ("  " + "  ".join(notes) if notes else "")
            ),
            "name": dst.name,
        }
...
def _trim_reference_file(path: Path, target_s: float = 25.0) -> str:
    ...
    try:
        ...
        return (
            f"Used the clearest {target_s:.0f}s (from {dur:.0f}s at "
...
def set_global_default_voice(candidate_path: str, transcript: str) -> dict:
    ...
    try:
        ...
        return {
            "ok": True,
            "message": "✅ Candidate promoted to the global default narrator.",
            "path": str(dst),
        }
    except Exception as e:  # noqa: BLE001
...
def generate_candidates(
    target: str, model_id: str, count: int = 3, text: str | None = None, progress=None
) -> dict:
    ...
    for i in range(count):
        ...
        req = GenRequest(
            ...
            reference_wav=None,
            reference_text=None,  # built-in voice each time
...
def _save_candidate_ratings(ratings: dict):
    VOICES.mkdir(parents=True, exist_ok=True)
    CANDIDATE_RATINGS_FILE.write_text(
...
def run_consistency_test(target: str, model_id: str, candidate_path: str, progress=None) -> dict:
    ...
    for i, line in enumerate(CONSISTENCY_TEST_LINES):
        ...
        req = GenRequest(
            ...
            target=target,
            language=TARGET_LANG.get(target, "en"),
            reference_wav=candidate_path,
            reference_text=None,  # controllable clone
            preset=preset_for_style("natural"),
        )
        try:
            r = get_router().generate(mid, req.to_json(), unload_after=False)
    ...
    if not paths:
        return {
            "ok": False,
            "message": f"Consistency test failed: {errors[:1]}",
            "paths": [],
            "errors": errors,
        }
    return {
        ...
        "paths": paths,
        "errors": errors,
        "message": f"Consistency test done: {len(paths)}/5 lines generated.",
    }
...
def ensure_default_voice(target: str, model_id: str, progress=None) -> dict:
    ...
    req = GenRequest(
        text=f"({DEFAULT_VOICE_PERSONA}) {text}",
        out_path=str(dst),
        target=DEFAULT_VOICE_TARGET,
        language=TARGET_LANG.get(DEFAULT_VOICE_TARGET, "en"),
        reference_wav=None,
        reference_text=None,  # built-in voice for the seed clip
        preset=preset_for_style("natural"),
    )
    r = get_router().generate(mid, req.to_json(), unload_after=False)
    if not r.get("ok") or not dst.exists():
        return {
            "ok": False,
            "message": f"Default voice generation failed: " f"{r.get('error', 'unknown')}",
        }
