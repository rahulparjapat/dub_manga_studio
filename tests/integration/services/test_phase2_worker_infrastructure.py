...
from chatterbox_manga_studio.services.worker_pool import WorkerDescriptor, WorkerMatchCriteria, WorkerPool
...
async def test_phase2_worker_pool_runtime_and_gpu_scheduler_integrate():
    ...
    await registry.register(ExistingWorkerPlugin(WorkerPluginConfig(
        model_id="echo", label="Echo", license_flag="test", estimated_vram=2,
        supported_languages=["en"], supports_voice_clone=False, supports_reference_text=False,
        supports_emotions=False, batch_support=True,
    )))
    ...
    pool = WorkerPool()
    await pool.register_worker(WorkerDescriptor(worker_id="echo-worker", capabilities=cap, gpu_id=allocation.gpu_id))
    ...
    result = await runtime.infer(RuntimeInferenceRequest(model_id="echo", payload={"text": "hello"}))
