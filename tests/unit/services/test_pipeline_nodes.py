...
async def test_ingest_node_copies_source_and_checkpoints(engine_and_storage, monkeypatch):
    ...
    run = await engine.execute(
        WorkflowDefinition(name="w", nodes=[WorkflowNode(id="ingest", handler="n")]),
        {"project_id": "p", "source_path": str(src)},
    )
...
async def test_transcribe_node_wraps_existing_whisper(engine_and_storage, monkeypatch):
    ...
    def fake_transcribe(
        ...
        (Path(out_dir) / "transcript.json").write_text(
            json.dumps([{"start": 0, "end": 1, "text": "hi"}]), encoding="utf-8"
        )
    ...
    src.write_bytes(b"v")
    run = await engine.execute(
        WorkflowDefinition(name="w", nodes=[WorkflowNode(id="transcribe", handler="n")]),
        {"project_id": "p", "source_video": str(src)},
    )
...
async def test_translation_quality_voice_nodes(engine_and_storage, monkeypatch):
    ...
    ref = P.VOICES / "v.wav"
    ref.write_bytes(b"not audio but voicecheck is lenient on errors")
...
async def test_tts_cleanup_render_export_dry_run(engine_and_storage, monkeypatch):
    ...
    monkeypatch.setattr(N, "project_dir", P.project_dir)
    src_dir = P.project_dir("p") / "source"
    src_dir.mkdir(parents=True)
    src = src_dir / "v.mp4"
    src.write_bytes(b"video")
    tr_dir = P.project_dir("p") / "transcript"
    tr_dir.mkdir(parents=True)
    (tr_dir / "transcript.json").write_text(
...
async def test_pipeline_node_failure_propagates_and_records_checkpoint(engine_and_storage):
    ...
    run = await engine.execute(
        WorkflowDefinition(name="w", nodes=[WorkflowNode(id="translation", handler="translation")]),
...
async def test_pipeline_node_cancellation_records_checkpoint(engine_and_storage):
    ...
    run = await engine.create_run(
        WorkflowDefinition(name="w", nodes=[WorkflowNode(id="slow", handler="slow")]),
        {"project_id": "p", "source_path": "x"},
    )
