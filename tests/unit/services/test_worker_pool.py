...
async def test_worker_pool_matches_reserves_releases_and_load_balances():
    ...
    await pool.register_worker(
        WorkerDescriptor(worker_id="w2", capabilities=cap, max_reservations=2)
    )

    matches = await pool.match_workers(
        WorkerMatchCriteria(language="en", supports_voice_clone=True)
    )
...
async def test_worker_pool_health_monitor_marks_unhealthy():
    ...
    await pool.register_worker(
        WorkerDescriptor(worker_id="w", capabilities=cap), health_check=lambda worker: False
    )
