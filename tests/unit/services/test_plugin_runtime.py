...
async def test_http_worker_adapter_wraps_existing_base_worker(tmp_path):
    ...
    thread.start()
    try:
        cap = ExistingWorkerPlugin(WorkerPluginConfig(
            model_id="http_model", label="HTTP", license_flag="test", estimated_vram=1,
            supported_languages=["en"], supports_voice_clone=False, supports_reference_text=False,
            supports_emotions=False, batch_support=False,
        )).capabilities
        adapter = HTTPWorkerAdapter(cap, f"http://127.0.0.1:{server.server_address[1]}")
        runtime = WorkerRuntime(adapter)
        out = tmp_path / "x.wav"
        result = await runtime.infer(RuntimeInferenceRequest(model_id="http_model", payload={"text": "x", "out_path": str(out), "target": "english"}))
...
async def test_plugin_runtime_factory_uses_metadata_for_transcription_adapter():
    ...
    await registry.register(ExistingWorkerPlugin(WorkerPluginConfig(
        model_id="transcriber", label="Transcriber", license_flag="test", estimated_vram=1,
        supported_languages=["*"], supports_voice_clone=False, supports_reference_text=False,
        supports_emotions=False, batch_support=True, metadata={"task": "transcription"},
    )))
