...
from chatterbox_manga_studio.dubbing.workers.protocol import TARGET_LANG, GenRequest
from chatterbox_manga_studio.export import srt as SRT
from chatterbox_manga_studio.export import timeline as TL
from chatterbox_manga_studio.export.subtitle_mask import _safe_blur_radii, build_mask_filter


def test_config_loads_and_profiles():
    cfg = C.load_config()
    assert "dubbing_models" in cfg
    assert {"chatterbox", "indicf5", "voxcpm2", "vibevoice", "fish"} <= set(cfg["dubbing_models"])
...
def test_timeline_cue_locked_NO_GAPS_default():
    ...
    cues = [TL.Cue(0, 0.0, 5.0, audio_seconds=3.0), TL.Cue(1, 6.0, 10.0, audio_seconds=2.0)]
...
def test_timeline_keep_natural_pauses_mode():
    cues = [
        TL.Cue(0, 0.0, 5.0, audio_seconds=3.0),
...
def test_timeline_silence_compression_in_keep_pauses():
    cues = [
        TL.Cue(0, 0.0, 1.0, audio_seconds=1.0),
        TL.Cue(1, 6.0, 7.0, audio_seconds=1.0),
    ]  # 5s gap
    tl = TL.build_timeline(
        "Cue-Locked (Keep Natural Pauses)", cues, compress_gaps_ms=1000, keep_after_ms=300
    )
...
def test_glossary_first_mapping_retained(tmp_path, monkeypatch):
    ...
    glossary.merge("proj", "hinglish_roman", {"characters": {"李明": "Li Ming"}})
...
def test_assemble_adaptation_and_forward(tmp_path, monkeypatch):
    ...
    for b in BM.load_plan(pid, t)["batches"]:
        v = BM.save_batch_version(
            pid, t, b["batch"], [f"b{b['batch']}-{j}" for j in range(b["cue_count"])], "gemini", "m"
        )
        BM.mark(pid, t, b["batch"], status="Done", active_version=v)
    asm = BM.assemble_adaptation(pid, t)
    assert len(asm) == 4
    PKG.forward_package(pid, t, {"narration_lines": asm, "dubbing_model": "indicf5"})
...
def test_emotion_adapter_model_aware():
    ...
    assert {"hype", "tense", "whisper", "sad", "comedic", "calm"} <= set(E.MANGA_PALETTE)
...
def test_reference_check_short_clip(tmp_path):
    ...
    sf.write(
        str(p), (0.3 * np.sin(2 * np.pi * 220 * np.arange(24000) / 24000)).astype("float32"), 24000
    )  # 1s
...
def test_stageflow_guidance_and_state(tmp_path, monkeypatch):
    ...
    for stg in ["adaptation", "transcribe", "dubbing", "export"]:
        ...
        assert {"stage", "current_gpu", "recommended", "ok", "why", "tip"} <= set(g)
...
def test_gpu_config_check_shape():
    ...
    assert {"configured", "detected", "match", "message"} <= set(c)
...
def test_cue_payload_carries_seconds_and_number():
    ...
    cues = [{"start": 0.0, "end": 2.5, "text": "你好"}, {"start": 2.5, "end": 6.0, "text": "世界"}]
...
def test_glossary_extract_and_lock():
    ...
    txt = '{"cues":[{"n":1,"text":"x"}],' '"glossary":{"characters":{"叶凡":"Ye Fan"},"powers":{}}}'
...
def test_backcheck_parse_and_summary():
    ...
    txt = '{"checks":[{"n":1,"ok":true,"issue":""},' '{"n":2,"ok":false,"issue":"name wrong"}]}'
...
def test_L1_placeholder_keeps_tail(tmp_path, monkeypatch):
    ...
    for i in (0, 2):
        sf.write(
            str(cue_dir / f"cue_{i:04d}.wav"), np.zeros(int(0.5 * 48000), dtype="float32"), 48000
        )
...
def test_L3_transcript_fingerprint(tmp_path, monkeypatch):
    ...
    tr.write_text(
        _j.dumps([{"start": 0.0, "end": 1.0, "text": "a"}, {"start": 1.0, "end": 2.0, "text": "b"}])
    )
...
def test_optimized_prompt_flows_into_json_parser():
    ...
    cues = [
        {"start": 0.0, "end": 2.5, "text": "叶凡突破了。"},
        {"start": 2.5, "end": 3.7, "text": "震惊！"},
    ]
    sysp = P.build_effective_prompt(
        "hinglish_roman", "Engaging YouTube Hinglish", "hype manhua", glossary=glossary
    )
    ...
    resp = (
        '{"cues":[{"n":1,"text":"Ye Fan ne breakthrough kiya."},'
...
def test_mux_with_bgm_builds_three_input_command(monkeypatch):
    ...
    EX.mux_audio_with_bgm(
        Path("v.mp4"), Path("n.wav"), Path("b.mp3"), Path("out.mp4"), "[1:a][2:a]amix[aout]"
    )
    cmd = captured["cmd"]
...
def test_direct_synth_signature_has_adapt():
    ...
    from chatterbox_manga_studio.directaudio.direct import adapt_direct_text, synth_direct
...
def test_retention_presets_in_prompt():
    ...
    p_en = P.build_effective_prompt("english", "Calm but Engaging", "", retention="Reaction / Hype")
    p_hi = P.build_effective_prompt(
        "hinglish_roman", "Calm but Engaging", "", retention="Reaction / Hype"
    )
...
def test_setup_presets_roundtrip(tmp_path, monkeypatch):
    ...
    P.save_setup_preset(
        "Manhua Hinglish",
        {
            ...
            "style": "Engaging YouTube Hinglish",
            "retention": "Cliffhanger",
            "add_emotions": True,
        },
    )
...
def test_qwen3tts_registered():
    from chatterbox_manga_studio.common.config import all_models, load_config, model_cfg
...
def test_router_http_surfaces_worker_error_body(monkeypatch):
    ...
    body = _j.dumps(
        {"ok": False, "error": "boom in synth", "trace": "Traceback: ValueError"}
    ).encode()
...
def test_worker_merge_guarantees_few_cues():
    ...
    seg = [
        {"id": i, "start": i * 1.5, "end": (i + 1) * 1.5, "text": f"s{i}", "words": []}
        for i in range(40)
    ]  # 60s of 1.5s cues
...
def test_pause_aware_snaps_to_natural_pause():
    ...
    while t < 60.0:
        ...
        if 22.5 <= t <= 23.5:  # a real pause near 23s
            gap = 0.4
        t = t + 0.5 + gap
    seg = [{"id": 0, "start": 0.0, "end": words[-1]["end"], "text": "完整句子", "words": words}]
...
def test_pause_aware_no_word_timings_fallback():
    ...
    ww = _load_worker_module()
    seg = [
        {"id": i, "start": i * 2, "end": (i + 1) * 2, "text": f"s{i}", "words": []}
        for i in range(20)
    ]
...
def test_mask_filter_supports_color():
    from chatterbox_manga_studio.export.subtitle_mask import MASK_COLORS, build_mask_filter
...
def test_package_latest_is_numeric_not_lexicographic(tmp_path, monkeypatch):
    ...
    for i in range(12):
        PKG.forward_package(
            "v", "hinglish_roman", {"narration_lines": [f"l{i}"], "dubbing_model": "indicf5"}
        )
...
def test_package_forward_uses_max_plus_one(tmp_path, monkeypatch):
    ...
    P.PROJECTS / "v"  # edition dir path resolution handled inside package
...
def test_intro_outro_edit_and_custom(tmp_path, monkeypatch):
    ...
    assert (
        IO.get_preset("Cliffhanger Hook")["intro"]
        == IO.BUILTIN_PRESETS["Cliffhanger Hook"]["intro"]
    )
    # add + delete custom
...
def test_live_render_cache_requires_exact_cue_duration_and_timing_mode():
    ...
    cues = [Cue(0, 0.0, 1.0, audio_seconds=1.25), Cue(1, 1.0, 2.0, audio_seconds=0.75)]
    ...
    assert not cache_matches_timeline(
        {"0": {"file": "old.mp4", "cue_ids": [0, 1]}}, timeline, "Cue-Locked Audio Master Sync"
    )
...
def test_reference_denoise_reduces_noise(tmp_path, monkeypatch):
    ...
    raw = tmp_path / "in.wav"
    sf.write(str(raw), speech + noise, sr)
...
def test_short_reference_kept_full(tmp_path, monkeypatch):
    ...
    importlib.reload(V)
    sr = 24000
    y = 0.3 * np.sin(2 * np.pi * 180 * np.linspace(0, 10, 10 * sr, endpoint=False)).astype(
        "float32"
    )
    raw = tmp_path / "s.wav"
    sf.write(str(raw), y, sr)
    V.save_uploaded_voice(str(raw), "sv", max_seconds=25.0)
...
def test_segment_uses_input_side_seek_and_cfr():
    ...
    assert (
        '"-ss", f"{seg.src_start:.3f}", "-t", f"{src_dur:.3f}",\n' in src
        or '"-ss", f"{seg.src_start:.3f}", "-t", f"{src_dur:.3f}"' in src
    )
...
def test_real_concat_duration_is_correct():
    ...
    subprocess.run(
        [
            ...
            "lavfi",
            "-i",
            "testsrc=size=640x360:rate=30:duration=12",
            "-pix_fmt",
            "yuv420p",
            "-c:v",
            "libx264",
    ...
    class FakeRouter:
        def generate(self, mid, req, unload_after=True):
            ...
            out = (
                ...
                else __import__("json").loads(req)["out_path"]
            )
...
def test_unlimited_voice_saves_autonumber(tmp_path, monkeypatch):
    ...
    importlib.reload(V)
    V.CANDIDATES_DIR = tmp_path / "_c"
    V.CANDIDATES_DIR.mkdir(parents=True)
    c = V.CANDIDATES_DIR / "cand.wav"
    sf.write(str(c), np.zeros(2400, dtype="float32"), 24000)
...
def test_transcribe_clip_coresident_when_vram_free(monkeypatch, tmp_path):
    ...
    class FakeRouter:
        def unload(self, mid=None):
            calls["unload"] += 1

    monkeypatch.setattr("chatterbox_manga_studio.dubbing.router.get_router", lambda: FakeRouter())
