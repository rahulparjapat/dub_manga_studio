...
def main():
    ...
    if not shutil.which("ffmpeg"):
        print("ffmpeg missing"); return 1
    filters = ["silenceremove=start_periods=1:start_silence=0.1:start_threshold=-45dB:"
               "stop_periods=1:stop_silence=0.2:stop_threshold=-45dB"]
    if not a.no_denoise:
        filters.append("afftdn=nf=-25")
    filters.append("loudnorm=I=-18:TP=-2:LRA=7")
    subprocess.run(["ffmpeg", "-y", "-i", a.input, "-af", ",".join(filters),
                    "-ac", "1", "-ar", str(a.sr), "-t", str(a.seconds), a.output], check=True)
    txt = Path(a.output).with_suffix(".txt")
    if not txt.exists():
        txt.write_text("<< type the exact words spoken in this clip here >>",
                       encoding="utf-8")
